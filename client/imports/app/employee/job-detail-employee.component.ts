...
import template from './job-detail-employee.component.html';
import style  from './job-detail-employee.component.scss';
...
@InjectUser('user')
export class JobDetailEmployeeComponent implements OnInit, OnDestroy{
    job: Job;
    jobId: string;
    ...
    ngOnInit(): void {
        this.paramsSub = this.route.params.map(params => params['jobId']).subscribe(jobId => {
            ...
            this.jobSub = MeteorObservable.subscribe('job-public', this.jobId).subscribe(() => {
                console.log('run');
