...
import template from './manage-jobs-employer.component.html';
import style from './manage-jobs-employer.component.scss';
...
@Component({
    selector: 'jobs-list-employer',
    template,
    styles: [style]
})

@InjectUser('user')
export class ManageJobsEmployerComponent implements OnInit, OnDestroy{
    jobs: Observable<Job[]>;
    ...
    ngOnInit(): void {
        ...
        this.jobsSub = MeteorObservable.subscribe('jobs', options, location).subscribe(()=>{
                this.jobs = Jobs.find({},{
                    sort: {
        ...
        this.autorunSub = MeteorObservable.autorun().subscribe(() => {
            this.jobsSize = Counts.get('numberOfJobs');
    ...
    removeJob(job: Job): void{
        if(window.confirm('Xóa tin đăng?')){
